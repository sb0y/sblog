...
class Vk {
    ...
    /**
     * Построение конечного URI для вызова
     * @param $method
     * @param string $params
     * @return string
     */
    ...
    public function get_code_token ($type="code")
    {
        ...
        $url = str_replace('{redirect_uri}', $this->callbackURL, $url);
        $url = str_replace('{display}', 'popup', $url);
    ...
    function upload_photo($gid = false, $files = array()){
        ...
        {
            $mime = getimagesize ($data);
            $mime = $mime["mime"];
            $files['file' . ($key+1)] = new CurlFile ( $data, $mime, $data ); // Sb0y[9-9-2013]: PHP 5.5.1 fix
        }
    ...
    function upload_video ($options = array(), $file = false)
    {
        ...
        if ( $file )
        {
            ...
            $files['video_file'] = new CurlFile ( $file, $mime, $file ); // Sb0y[9-9-2013]: PHP 5.5.1 fix;
            curl_setopt ( $ch, CURLOPT_POSTFIELDS, $files );
            curl_exec ( $ch );
